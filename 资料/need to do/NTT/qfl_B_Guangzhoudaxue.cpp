...
const int G = 3, mod = 998244353, Maxn = 3e5 + 10;
...
Poly mul(const Poly &A) {
	static int a[Maxn << 1];
	for (int i = 0; i < A.size(); i ++) a[i] = A[i];
	Poly C;
	C.resize(std::min(lim, (int)(A.size() + A.size() - 1)));
	int n = 1; 
	for(n; n < A.size() + A.size() - 1; n <<= 1);
	NTT(n, a, 1);
	px(n, a, a);
	NTT(n, a, 0);
	for (int i = 0; i < C.size(); i ++) {
...
void solve() {
	...
	const i64 B = std::pow(n / std::__lg(m), 0.33);
	std::vector<int> ans(2 * m + 1);
	lim = 2 * m + 1;
	// <= B
	for (int i = 1; i <= B; i ++) {
		Poly a(m + 1);
